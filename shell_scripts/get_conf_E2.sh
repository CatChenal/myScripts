...
cd $FLD
   ...
   echo $dataline >> tmps #$fileout

   strHeader="CONFORMER@occ@vdw0@vdw1@tors@epol@dsolv@h3_tot@res_b@res_u"
   sedstr=" -i '1i "$strHeader"' "
   eval sed $sedstr $fileout
   sed 's/@/\t/g' tmps | sort -k7n > $fileout

hdr=$( awk '/CONF/{print NR; exit}' $fileout )
if [[ $hdr -eq 1 ]]; then  # still row1 after sort =>no fav: keep first 4
   head -5 $fileout >> $fileout"neg_fav_h3.csv"
else
   sed '/h3_tot/q' tmp >> $fileout"neg_fav_h3.csv"
fi

  ...
  if [[ "$RES" =~ "CL" ]]; then
    ...
    if [[ ${#AnchorLine} ]]; then
      ...
      nawk -v x="$X" -v y="$Y" -v z="$Z" -v max="$cutoff" -v dir="$fldr" -v HIS="$mostH165" 'BEGIN{OFS="\t"}
          { if ($4 != "CL")  {dist=sqrt( (x-$7)^2 + (y-$8)^2 + (z-$9)^2 ); { if (dist<=max) { print dir,"Cl-1","CL",HIS,$4"_"$5"_"$6,dist }}} }' $mostocc >> $outfile
    else
      ...
      do
        ...
        if [[ ${#AnchorLine} ]]; then
          ...
          nawk -v x="$X" -v y="$Y" -v z="$Z" -v max="$cutoff" -v dir="$fldr" -v Pi="$mostPi" -v HIS="$mostH165" -v i="$n" 'BEGIN{OFS="\t"}
            { if ($4 != "PO4") { dist=sqrt( (x-$7)^2 + (y-$8)^2 + (z-$9)^2 ); { if (dist <= max) { print dir,HIS,Pi,"O"i,$4"_"$5"_"$6,dist }}} }' $mostocc>>  $outfile
        else
...
# Combine all frames:
cat $startwith*/*"-res-"$cutoff".csv" |sort -k1 -k5 -k6n | sed '1i dir      occAnchor       atmAnchor       occH165	Res     dist_anchor'  > $here"-allframes-res-"$cutoff"A.csv"
